...
async def test_submit_requires_auth(client: AsyncClient) -> None:
    resp = await client.post(
        "/prescriptions/submit", data={"medications": json.dumps([MEDICATION])}
    )
...
async def test_submit_rejects_empty_medications(
    ...
    resp = await client.post(
        "/prescriptions/submit", data={"medications": json.dumps([])}, headers=headers
    )
    assert resp.status_code == 422
...
async def test_submit_rejects_invalid_medications_json(
    ...
    resp = await client.post(
        "/prescriptions/submit", data={"medications": "not-json"}, headers=headers
    )
    assert resp.status_code == 422
    assert "message" in resp.json()
...
async def test_get_prescriptions_returns_only_own_newest_first(
    ...
    resp = await client.get("/prescriptions", headers=headers)
...
async def test_get_prescriptions_does_not_leak_other_users(
    ...
    resp_b = await client.get("/prescriptions", headers=headers_b)
