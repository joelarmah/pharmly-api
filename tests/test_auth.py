...
async def test_otp_request_rate_limited_on_cooldown(
    ...
    resp1 = await client.post("/auth/otp/request", json={"phone_number": PHONE})
    assert resp1.status_code == 204
    resp2 = await client.post("/auth/otp/request", json={"phone_number": PHONE})
...
async def test_login_locks_out_after_max_failed_attempts(
    ...
    locked_resp = await client.post("/auth/login", json={"phone_number": PHONE, "pin": "123456"})
...
async def test_pin_reset_happy_path(client: AsyncClient, fake_sms: FakeSmsSender) -> None:
    ...
    reset_resp = await client.post(
        "/auth/pin/reset", json={"verification_token": verification_token, "pin": "654321"}
    )
    assert reset_resp.status_code == 200

    login_resp = await client.post("/auth/login", json={"phone_number": PHONE, "pin": "654321"})
    assert login_resp.status_code == 200

    old_pin_resp = await client.post("/auth/login", json={"phone_number": PHONE, "pin": "123456"})
...
async def test_pin_verify_happy_path_and_mismatch(
    ...
    bad_resp = await client.post("/auth/pin/verify", json={"phone_number": PHONE, "pin": "000000"})
...
async def test_pin_change_requires_auth_and_correct_current_pin(
    ...
    relogin = await client.post("/auth/login", json={"phone_number": PHONE, "pin": "654321"})
...
async def test_token_refresh_rotates_and_invalidates_old_token(
    ...
    reuse_resp = await client.post("/auth/token/refresh", json={"refresh_token": refresh_token})
...
async def test_delete_me_removes_account(client: AsyncClient, fake_sms: FakeSmsSender) -> None:
    ...
    login_resp = await client.post("/auth/login", json={"phone_number": PHONE, "pin": "123456"})
